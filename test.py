...
def main():
    ...
    paired_amplifier = requests.get(
        'http://127.0.0.1:8080/get/paired/amplifier/tester@email.xx').json()
    ...
    change_volume = requests.post(
        'http://127.0.0.1:8080/change/volume/by/id', {"amplifier": tested_amplifier, "volume": 40})
    ...
    change_volume = requests.post(
        'http://127.0.0.1:8080/change/volume/by/email', {"email": "tester@email.xx", "volume": 80})
    if change_volume.status_code != 200:
        logging.critical('Error while changing volume')
        sys.exit(1)
