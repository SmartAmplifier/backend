...
def main():
    ...
    pair_new_amplifier = requests.post('http://localhost:8345/pair/new/amplifier',
    ...
    register_new_amplifier = requests.post('http://localhost:8345/register/new/amplifier',
    ...
    paired_amplifier = requests.get(
        'http://localhost:8345/get/paired/amplifier/tester@email.xx').json()
    ...
    change_volume = requests.post(
        'http://localhost:8345/change/volume/by/id', {"amplifier": tested_amplifier, "volume": 40})
    if change_volume.status_code != 200:
        logging.critical('Error while changing volume')
        sys.exit(1)

    volume = requests.get(
        'http://localhost:8345/get/volume/by/id/{}'.format(tested_amplifier)).json()['volume']
    ...
    change_volume = requests.post(
        'http://localhost:8345/change/volume/by/email', {"email": "tester@email.xx", "volume": 80})
    if change_volume.status_code != 200:
        logging.critical('Error while changing volume')
        sys.exit(1)

    volume = requests.get(
        'http://localhost:8345/get/volume/by/email/tester@email.xx').json()['volume']
